...
export default function Quiz({ onClose }) {
  ...
  const getRank = (score) => {
    if (score === 5)
      return {
        title: "Terminal Master (root)",
        desc: "You have complete command over the shell! Superuser status achieved.",
      };
    if (score >= 3)
      return {
        title: "Power User (sudo)",
        desc: "Excellent CLI skills. You navigate environments with ease and confidence.",
      };
    return {
      title: "User (guest)",
      desc: "Good start! Spend more time experimenting with interactive builders to master flags.",
    };
  };
        ...
        {!showResults ? (
          ...
          <div className="quiz-score-page">
            ...
            >
              ...
              <button className="quiz-next-btn" onClick={onClose}>
                Back to Tools
              </button>
            </div>
          </div>
